...
namespace PiF.Controllers
{
    ...
    public class EditGameGridController : Controller
    {
        ...
        public ActionResult DeleteAjaxEditing(int id)
        {
            ...
            // Rebind the grid
            return this.View(new GridModel(SessionEditGamesRepository.All()));
        }
        ...
        public ActionResult InsertAjaxEditing()
        {
            ...
            if (this.TryUpdateModel(pifgame) && this.ModelState.IsValid)
            {
                ...
                else if (SessionEditGamesRepository.One(p => p.ID == dbGame.id) != null)
                {
                    this.ModelState.AddModelError("Name", "Duplicate game, please edit existing row.");
                }
                else
                {
                    SessionEditGamesRepository.Insert(new PiFGame(pifgame.Count, dbGame));
                }
            }
        ...
        public ActionResult UpdateAjaxEditing(int id)
        {
            ...
            if (this.TryUpdateModel(pifgame) && this.ModelState.IsValid)
            {
                ...
                else if (SessionEditGamesRepository.One(p => p.ID == dbGame.id && dbGame.id != id) != null)
                {
                    this.ModelState.AddModelError("Name", "Duplicate game, please edit existing row.");
                }
                else
